...
describe("expanded assessment platform", () => {
  it("ships three unique assessment definitions", () => {
    expect(expansionAssessments).toHaveLength(3);
    expect(new Set(expansionAssessments.map((item) => item.id)).size).toBe(3);
  });
